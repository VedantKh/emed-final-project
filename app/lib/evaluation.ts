...
export function parseEvaluationResponse(
  ...
  try {
    ...
    const questionSections = responseText
      .split(/## Question \d+:/g)
      .filter(Boolean);

    if (questionSections.length === 0) {
      // Fallback approach - try to find scores directly
      const ethicalScores = responseText.match(
        /\*\*Ethical Score:\*\*\s*(\d+)/g
      );
      const strategicScores = responseText.match(
        /\*\*Strategic Score:\*\*\s*(\d+)/g
      );
      ...
      if (ethicalScores && strategicScores && ethicalScores.length > 0) {
        ...
        for (
          let i = 0;
          i < Math.min(ethicalScores.length, strategicScores.length);
          i++
        ) {
          const ethicalMatch = ethicalScores[i].match(/(\d+)/);
          const strategicMatch = strategicScores[i].match(/(\d+)/);
          ...
          const feedbackStartIndex =
            responseText.indexOf(strategicScores[i]) +
            strategicScores[i].length;
          const feedbackEndIndex =
            i < ethicalScores.length - 1
              ? responseText.indexOf(ethicalScores[i + 1], feedbackStartIndex)
              : responseText.length;

          const sectionText = responseText.substring(
            feedbackStartIndex,
            feedbackEndIndex
          );

          // Try to find feedback and comparison in this section
          const feedbackMatch = sectionText.match(
            /\*\*Feedback:\*\*([\s\S]*?)(?=\*\*|$)/
          );
          const comparisonMatch = sectionText.match(
            /\*\*Real-World Comparison:\*\*([\s\S]*?)(?=\*\*|$)/
          );

          const feedback = feedbackMatch
            ? feedbackMatch[1].trim()
            : "Extracted feedback not available";
          const comparison = comparisonMatch
            ? comparisonMatch[1].trim()
            : "Extracted comparison not available";
      ...
      for (const section of questionSections) {
        ...
        const strategicMatch = section.match(
          /\*\*Strategic Score:\*\*\s*(\d+)/
        );
        ...
        if (ethicalMatch && strategicMatch) {
          ...
          const comparisonMatch = section.match(
            /\*\*Real-World Comparison:\*\*([\s\S]*?)(?=##|$)/
          );
          const comparison = comparisonMatch
            ? comparisonMatch[1].trim()
    ...
    if (questionScores.length === 0) {
      console.error(
        "Failed to parse with primary and secondary methods. Response format:",
        responseText
      );
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert in healthcare executive leadership evaluation.",
        },
        {
          role: "user",
          content: prompt,
